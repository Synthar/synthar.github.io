	...
	if ($('#contactForm').length > 0 ) {
		$( "#contactForm" ).validate( {
			...
			messages: {
				...
				email: "Merci d’entrer votre courriel",
			...
			submitHandler: function(form) {		
				...
				$.ajax({   	
			      ...
			      error: function() {
			      	$('#form-message-warning').html("Mince ! Il y a eu un problème… Veuillez essayer de me contacter par un autre moyen s’il vous plait :)");
			        $('#form-message-warning').fadeIn();
			         $submit.css('display', 'none');
			      }
		      });    		
